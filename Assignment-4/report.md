...
* **Two random seeds: 7 and 123** are run independently per image to expose initialization sensitivity.
* The output is a label map (one integer per pixel in `{0, 1, 2, 3}`). Cluster IDs are remapped in order of mean brightness (darkest cluster = 0, brightest = 3) so that the same ID means the same kind of region across seeds and across images. Each label map is saved as an indexed PNG colored with the Pascal VOC palette, matching the visual style of the Task 2 predictions.

### Sample results

A three panel figure (original, seed 7, seed 123) is saved for every image in `results/task1_kmeans/<id>/comparison.png`. Palette mapping: black = darkest cluster, dark red = 2nd, dark green = 3rd, olive = brightest.
...
| `results/task1_kmeans/2011_002730/comparison.png` | Horse on field. A clear example of seed sensitivity: seed 7 splits the foreground field into "shadow" and "lit ground" clusters, while seed 123 merges them into a single red cluster and uses the fourth cluster on fine tree texture instead. |
...
| Device | CPU |
| Total training time | about 5 min 16 s (316 s) |
...
| `2011_000068_compare.png` | Bird (class 3, seen in training) | Excellent. mIoU 0.623 |
| `2011_000054_compare.png` | Person | Good. mIoU 0.598 |
| `2011_000116_compare.png` | Aeroplane | Good. mIoU 0.498 |
| `2011_000112_compare.png` | Bird | Decent. mIoU 0.362 |
| `2011_000069_compare.png` | Person plus cow (cow unseen in training) | Mixed. mIoU 0.358 |
| `2011_000070_compare.png` | Difficult scene | Mediocre. mIoU 0.330 |
| `2011_000105_compare.png` | Contains train (unseen in training) | Weak. mIoU 0.170 |
| `2011_000006_compare.png` | Multi-object | Weak. mIoU 0.155 |
...
* **Pixel accuracy: 0.8452**
* **Mean IoU (over classes present in the test set): 0.1989**

**Per image:**

| Image | Pixel acc. | mIoU | Comment |
|---|---|---|---|
| 2011_000006 | 0.551 | 0.155 | multi-object |
| 2011_000051 | 0.934 | **0.856** | person, best result |
| 2011_000054 | 0.946 | 0.598 | person |
| 2011_000068 | 0.991 | 0.623 | bird |
| 2011_000069 | 0.955 | 0.358 | contains cow (unseen) |
| 2011_000070 | 0.801 | 0.330 | sofa (seen), hard viewing angle |
| 2011_000105 | 0.826 | 0.170 | contains train (unseen) |
| 2011_000108 | **0.101** | **0.021** | cat (unseen). failure |
| 2011_000112 | 0.963 | 0.362 | bird, small object |
...
| Class | IoU | Notes |
|---|---|---|
| 0 background | 0.854 | dominant, high IoU is expected |
| 15 person | 0.689 | present in many training images |
| 18 sofa | 0.549 | seen in training, single large region typically |
| 3 bird | 0.494 | seen in training, usually small but distinctive shape |
...
* **Pixel accuracy (0.85) is optically flattering but misleading.** Pascal VOC scenes are about 70 percent background. A trivial "predict background everywhere" model already scores about 70 percent, so 85 percent is only a moderate beat.
...
* **Where the model succeeded.** `person` (IoU 0.69), `bird` (0.49), and `background` (0.85) are all present in several training images and are all predicted well. That is the region of the class space where the method actually works.
...
* **Good performance on well represented classes.** Person at 0.69 IoU and bird at 0.49 IoU are both serviceable for a 20 image training set.
...
* **Compute cost.** About 5 min of CPU training. K-means is roughly free per image.
...
| Performance metric | None directly meaningful | Pixel acc 0.85, mIoU 0.20 |
